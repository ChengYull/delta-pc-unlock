...
| 模式 | 轮数 | 单轮限时 | 出题方式 | 计入成绩 | 排行榜 |
| --- | --- | --- | --- | --- | --- |
| 练习 | 1 | 不限 | 摩斯图案 | ❌ | — |
| 挑战 | 15 | 5 秒 | 摩斯图案 | ✅ | 挑战排行榜 |
| 音频 | 5 | 不限 | **只听声音** | ✅ | 音频排行榜 |

难度按钮在标题右边共处一行，共练习 / 挑战 / 音频三个；窄屏（≤440px）下连成分段控件，
并把标识图收到 30px、标题降到 16px —— 360px 窗口里顶栏可用区只有 234px
（右侧 94px 要让给宿主自带的悬浮按钮），三个按钮刚好放得下、余 8px。
...
- **音频**：连续 5 轮、**不限时**，考的是听力。每轮开始自动把 3 位摩斯按位顺序播报
  **最多 3 遍**（位间 300ms、遍间 700ms），界面上**不显示点杠图案**，每行改显一条振幅条。
  播放过程中键盘全程可用：抢答出 3 位就立刻判定并掐掉剩余播报，所以手快的人听一遍也能拿分——
  音频榜按总时间排序，抢答省下的时间是实打实的优势。振幅条由 AnalyserNode 的**真实电平**驱动，
  播到哪一位哪一行的条就起伏，玩家能从亮灭节奏里看出长短（这是明知的设计取舍）。
...
- 对照表（密码破译线索）常驻显示，与游戏一致：图案在左、数字在右，顺序 1-9 再 0。
  三种模式都保留——它是通用规则表，不含本轮答案。
- 看图模式下每行摩斯有「试听」按钮；**音频模式下这一列换成振幅条，试听按钮收起**，
  否则等于无限次重听，「最多 3 遍」的限制就废了。
...
npm run hb-sdk -- remote cloud leaderboard create audio_challenge  --order desc --rank-limit 500
```

> **每个小程序最多 3 个排行榜。** 模式从「新手 / 熟练 / 专家」三档收敛成「练习 / 挑战」后，
> `novice_challenge`、`skilled_challenge` 这两个 v3 遗留榜就不再被读写，却仍占着名额——
> 要加音频榜得先把它们删掉：`npm run hb-sdk -- remote cloud leaderboard delete <key>`
> （**硬删除，记录一并清掉**）。本地记录侧不用管：`normalizeRecord()` 只保留
> `CHALLENGE_ORDER` 里还有的 id，被删掉的两档记录下次存盘时自动消失。
...
最小窗口 360px 下留给「图标 + 文案 + 三个难度」只有 234px，所以标题压到 4 个字、
副标题压到 6 个字（`.brand__text` 也带省略号兜底）；440px 以下还把标识图收到 30px、
标题降到 16px、三个 chip 连成一段（无间隙、共享分隔线）：

| 窗口 | 顶栏需要 | 可用 | 余量 |
| --- | --- | --- | --- |
| 520px | 285px | 394px | 109px |
| 480px（默认） | 285px | 354px | 69px |
| 441px | 285px | 315px | 30px |
| 440px（断点） | 226px | 314px | 88px |
| 390px | 226px | 264px | 38px |
| 360px（最小） | 226px | 234px | **8px** |

**360px 只剩 8px**，所以加第四个模式按钮之前必须先量一遍；改标题或副标题的长度同样要按这张表算，
超了就会挤成省略号。
...
src/morse.js          摩斯数字表、编解码、Web Audio 播放器（含实时电平给振幅条用）
src/challenge.js      三种模式配置、榜单分数编解码、成绩比较
...
- 音效依赖宿主 WebView 的 Web Audio。取不到 AudioContext 或调度异常时会静默降级为无声，
  不会阻塞回合——但音频模式下的振幅条不会因此失效：`morsePlayer.level()` 读不到真实波形时
  会退回按「播放调度时间线」给 0/1，节奏照样对得上。
