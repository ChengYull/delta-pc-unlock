...
所以榜单上每个人的昵称与头像只能由各人自己在提交成绩时写进 `extra`：

```
extra = { solved, totalMs, nickname, avatar }      // nickname ≤ 20 字，avatar ≤ 512 字节
```

`user.getInfo()`（需声明 `userInfo` 权限，声明即可用、无需平台批准）静默读取当前用户的
`profile.nickname` / `profile.avatar`，结果在会话内缓存，失败不缓存以便下次重试。资料读取
失败时只提交成绩，不影响主流程。头像地址来自小黑盒自家 CDN，落在平台 CSP 的
`img-src` 白名单（`*.xiaoheihe.cn` / `*.max-c.com` 等）内。

两个随之而来的边界：

- **记录只在成绩更优时更新**，`extra` 同理。所以改了昵称或头像后，要等下次破纪录才会同步到榜单。
- 没有资料的历史记录仍然显示 `玩家 + appUserId 前 6 位`；头像加载失败则退回首字母方块。
